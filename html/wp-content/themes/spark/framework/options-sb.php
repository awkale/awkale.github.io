...
class sidebar_generator {
	...
	function admin_menu(){
		add_submenu_page('options-framework', __('Sidebars','csc-theme'), __('Sidebars','csc-theme'), 'edit_theme_options', 'sidebars', array('sidebar_generator','admin_page'));
	}
				...
				<div class="sbg_container">
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php  echo $i; ?>]" hidden="hidden" style="display:none">
								<option value="0"<?php  if($selected_sidebar[$i] == ''){ echo " selected";} ?> >Default</option>
							</select>

							<select name="sidebar_generator_replacement[<?php  echo $i; ?>]">
								<option value="0"<?php  if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>>Default WP</option>
							<?php 
